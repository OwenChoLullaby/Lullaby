...
function Node(id, pid, name, url, title, devUser, date ,target, icon, iconOpen, open) {
	this.id = id;
	this.pid = pid;
	this.name = name;
	this.url = url;
...
dTree.prototype.add = function(id, pid, name, url, title, devUser, date, target, icon, iconOpen, open) {
	this.aNodes[this.aNodes.length] = new Node(id, pid, name, url, title, devUser, date, target, icon, iconOpen, open);
};
...
// close depth
dTree.prototype.closeFold = function(depth){
	var id = [];
	for (var n=0; n<this.aNodes.length; n++) {
		if(this.aNodes[n]._hc != false&& this.aNodes[n].id.length > 5){
			id.push(this.aNodes[n].id)

			if (this.aNodes[n].id != this.aNodes[n]._p.id){

				if(id.indexOf(this.aNodes[n].pid) >= 0){
					var ai = this.aNodes[n]._p._ai;

					this.nodeStatus(true, ai, this.aNodes[ai]._ls);
					this.aNodes[ai]._io = true;
...
dTree.prototype.closeLv = function(level){
	this.nodeStatus(false, level, this.aNodes[level]._ls);
	this.aNodes[level]._io = false;
}
...
dTree.prototype.node = function(node, nodeId) {
	...
	var devName = '';
	if (node.devUser) devName += '작업자 : '+ node.devUser;

	var devDate = '';
	if (node.date) devDate += '최종 작업일 : '+ node.date;
	...
	if (node.url) {
		...
		str += ' '+color + 'data-state="'+state+'"';

		str += '>';
		str += node.name;
		if (node.url || ((!this.config.folderLinks || !node.url) && node._hc)) str += '</a>';
		str+='<div class="progress-info"><span>화면 ID : ' + node.name + '</span><span>' + devName + '</span><span>' + devDate + '</span></div>';
	}else if ((!this.config.folderLinks || !node.url) && node._hc && node.pid != this.root.id){
		if (node.name.indexOf('완료') !== -1) {
			str += '<a href="javascript: ' + this.obj + '.o(' + nodeId + ');" class="node complete" '+color+ 'data-state="'+state+'"'+'>';
		}else{
			str += '<a href="javascript: ' + this.obj + '.o(' + nodeId + ');" class="node" '+color+ 'data-state="'+state+'"'+'>';
		}
		str += node.name;
		if (node.url || ((!this.config.folderLinks || !node.url) && node._hc)) str += '</a>';
	} else {
		str += '<a href="javascript:;" '+color+ 'data-state="'+state+'"'+' id="s' + this.obj + nodeId + '">';
		str += node.name;
		if (node.url || ((!this.config.folderLinks || !node.url) && node._hc)) str += '</a>';
	}
...
dTree.prototype.oAll = function(status) {
	...
	if(status == true) this.className = "active"
	else this.className = ""
};
